#taken from https://github.com/Ahmed512w/Python-PostgreSQL-CRUD
...
class Crud:
    ...
    def update_count(self, column, primaryKey_value):
        update_query  = sql.SQL("UPDATE {} SET {} = {} + 1 WHERE {} = {}").format(
            sql.Identifier(self.table),
            sql.Identifier(column),
            sql.Identifier(column),
            sql.Identifier(self.primarykey),
        ...
        print(update_query)
        self._execute(update_query, ( primaryKey_value,))
        self._counter += 1
    ...
    def filterData(self, columnFilter):
        filter_query  = sql.SQL("SELECT * FROM {} ORDER BY {}").format(
            sql.Identifier(self.table),
            sql.Identifier(columnFilter)
        )
        self._execute(filter_query)
